...
const ZoneModuleSetup = () => {
  const IncreaseDeliveryChargeRef = useRef(null);
  const IncreaseDeliveryChargeMessageRef = useRef(null);
            ...
            <div className="md:w-full w-fit">
              <div className="w-full justify-between flex items-center">
                ...
                <Switch
                  size="medium"
                  onChange={() =>
                    (IncreaseDeliveryChargeRef.current.disabled =
                      !IncreaseDeliveryChargeRef.current.disabled)
                  }
                />
              </div>
            ...
            <div className="md:w-full w-fit flex flex-col gap-2">
              <div className="w-full justify-between flex items-center">
                ...
                <Switch
                  size="medium"
                  onChange={() =>
                    (IncreaseDeliveryChargeMessageRef.current.disabled =
                      !IncreaseDeliveryChargeMessageRef.current.disabled)
                  }
                />
              </div>
              <input
                ...
                disabled={true}
                ref={IncreaseDeliveryChargeMessageRef}
              ></input>
            </div>
          </div>
          <div className="flex flex-col gap-2">
            <div className="flex flex-col gap-1 text-[2vh]">
              <h1>Business Module's</h1>
            </div>
            <div className="flex w-fit md:w-full gap-1 border h-10 rounded px-2 py-1 text-[1.9vh]">
              {cookies?.adminuser?.user?.module?.map((item, index) => {
                return (
                  <h1 className="bg-emerald-100 py-1 px-4" key={index}>
                    {item?.module}
                  </h1>
                );
              })}
